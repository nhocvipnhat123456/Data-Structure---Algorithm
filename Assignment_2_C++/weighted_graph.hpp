...
class weighted_graph {
	...
	std::unordered_set<vertex> get_vertices() const; //Returns a vector containing all the vertices.
	void copy_graph(const weighted_graph& ); //Copy a graph
...
template <typename vertex>	std::unordered_set<vertex> weighted_graph<vertex>::get_vertices() const{
	return vertices;
}

template <typename vertex>	void weighted_graph<vertex>::copy_graph(const weighted_graph& g){
	adj_list = g.adj_list;
	vertices = g.vertices;
	m = g.m;
	n = g.n;
